...
public abstract class MyThreadPoolExecutor extends ThreadPoolExecutor {
	...
	private SocketConnection memstallSocket = new SocketConnection("127.0.0.1", 12003);
...
//		executor.attachVtune();
	...
	public void execute(Runnable command) {
		...
		if (topSocket.getSocketConnection() == null) {
			log.debug(String.format("[Socket][Top] Still could not connect to top socket, retrying..."));
			topSocket.connect();
		}
		
		if (memstallSocket.getSocketConnection() == null) {
			log.debug(String.format("[Socket][MemStall] Still could not connect to memstall socket, retrying..."));
			memstallSocket.connect();
		}
		...
		if (command instanceof TaskRunner) {
			...
			if (getCurrentStage() != taskRunner.getStageId()) {
				...
				setCurrentStage(taskRunner.getStageId());
				
				
				log.debug(String.format("[Socket][All] Letting all the listening sockets know the stage has changed"));
				topSocket.getSocketConnection().notifyStageChanged(taskRunner.getStageId());
				memstallSocket.getSocketConnection().notifyStageChanged(taskRunner.getStageId());
				mpstatSocket.getSocketConnection().notifyStageChanged(taskRunner.getStageId());
				iostatSocket.getSocketConnection().notifyStageChanged(taskRunner.getStageId());
			}
		}
	}
